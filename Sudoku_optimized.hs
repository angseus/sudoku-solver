...
isOkayBlock []     = True
isOkayBlock (x:xs) = (not (x `elem` xs) || x == Nothing) && isOkayBlock xs
-- Second solution use less allocation but they are almost as fast
...
prop_Blank sud = emptyCell == Nothing
  where (Pos(row, cell)) = blank sud
        emptyCell = head $ drop (row*9+cell) $ cells sud

prop_NewBlank :: Sudoku -> Bool
prop_NewBlank sud = emptyCell == Nothing
  where (Pos(row, cell)) = newBlank sud
        emptyCell = head $ drop (row*9+cell) $ cells sud

newBlank :: Sudoku -> Pos
newBlank (Sudoku sud) = snd 
                      $ head 
                      $ sort 
                      $ newBlank' (Sudoku sud) blankXs
  where blankXs = [ Pos (x `div` 9, x `mod` 9) 
                  | x <- [0..80]
                  , cellList !! x == Nothing ]
        cellList = concat sud

newBlank' :: Sudoku -> [Pos] -> [(Int, Pos)]
newBlank' (Sudoku sud) xs = [ (priorityRow x, x)    | x <- xs ] 
                         ++ [ (priorityColumn x, x) | x <- xs ]
                     --    ++ [ (priority3by3 x, x)   | x <- xs ]
  where priorityRow    (Pos(row, cell)) = count Nothing $ concat $ take 1 $ drop row sud
        priorityColumn (Pos(row, cell)) = count Nothing $ concat $ take 1 $ drop cell $ transpose sud
        --priority3by3 (Pos(row, cell)) = 
        count _ []     = 0
        count y (x:xs) = if x == y
                       then (count y xs) + 1
                       else count y xs
...
(!!=) :: [a] -> (PosInList,a) -> [a]
(!!=) xs ((PosInList(pos)), val) = take pos xs ++ [val] ++ drop (pos+1) xs

-- New datatype for positions in a list of cells
data PosInList = PosInList (Int)
  deriving (Show, Eq, Ord)

instance Arbitrary PosInList where
  arbitrary = do n <- choose (0,81)
                 return (PosInList (n))

prop_Operator :: (Eq a) => [a] -> (PosInList, a) -> Property
prop_Operator xs (PosInList(pos), val) = length xs >= pos && pos >= 0 ==> (ys !! pos) == val
  where ys = xs !!= (PosInList(pos), val)
...
update (Sudoku sud) (Pos(row, cell)) (Cell val) = (Sudoku updated)
  where updated = take row sud ++ 
                  [((concat $ take 1 $ drop row sud) !!= ((PosInList(cell)), val))] ++ 
                  drop (row+1) sud
...
solve sud
  ...
  | otherwise              = listToMaybe $ catMaybes $ pmap solve newSuds
  where newSuds            = [update sud (newBlank sud) (Cell (Just a)) | a <- [1..9]]
...
isSolutionOf a b  = isSudokus 
                 && solution 
                 && (and [ (fst z == snd z) || (snd z == Nothing) | z <- zipped])
  where zipped    = zip (cells a) (cells b)
        solution  = isSolved a && (a == (fromJust $ solve b))
        isSudokus = isSudoku a && isSudoku b
